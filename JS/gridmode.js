...
function colorCellInit(){
    ...
    let trueArea = +getArea.value * +getArea.value

    for (let row = 0; row < +getArea.value; row++) {
        collumnsWantedTL += `"`
        for (let i = 0; i < +getArea.value; i++) {
    ...
    cellContainer.children[+getArea.value - 1].className += "nomove"
    cellContainer.children[number - +getArea.value].className += "nomove"
...
function scrambleColors(){
    let scrambleCycle = +getArea.value * 4
